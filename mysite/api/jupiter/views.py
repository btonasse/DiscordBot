...
class BaseCharacterView(generics.GenericAPIView):
    queryset = md.Character.objects.select_related("killed_by", "klass", "challenge").prefetch_related(
        "awards",
        "effects",
        ...
        Prefetch("characterequipment_set", queryset=md.CharacterEquipment.objects.select_related("equipment").prefetch_related(Prefetch("equipmentperk_set", queryset=md.EquipmentPerk.objects.select_related("perk")))),
    )
